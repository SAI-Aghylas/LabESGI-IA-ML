...
def update(q_table, state, action,r, Gamma, stateNext, actionNext):
    ...
    q_table[state][action] = q_table[state][action] + 0.5*(r + (Gamma *q_table[stateNext][actionNext] - q_table[state][action]))
    return q_table

def choose_action(state, q_table, epsilon):
    if epsilon > random.uniform(0, 1): #  take a random number and compare to exploratio rate
...
def reduce_epsilon(epsilon):
    ...
    epsilon = epsilon * 0.30  # reduce epsilon after each step
    epsilon = max(epsilon,0.10) # set a minimum value for epsilon
    return epsilon
